...
Original file is located at
    https://colab.research.google.com/drive/1sx8KXT9NrCw93vIlBTNyF9RoNUNNMYDN
...
def viz_uniform_teoretis(lower_bound, upper_bound, num_samples):
  ...
  resultData = generate_rand_uniform(
      lower_bound,
      upper_bound,
      num_samples
    )
...
def generate_data_synthetic(breed, features, n_samples, params):
  ...
  for feature in features:
      if feature in ["wingspan_cm", "weight_g"]:
          dist = params[breed][feature]
          df[feature] = generate_gaussian(dist.miu, dist.sigma, n_samples)
      elif feature == "sing_days":
          dist = params[breed][feature]
          df[feature] = generate_binomial(dist.n_trials, dist.probability, n_samples)
      elif feature == "beak_head_ratio":
          dist = params[breed][feature]
          df[feature] = generate_rand_uniform(dist.lower_bound, dist.upper_bound, n_samples)
      else:
...
def pdf_for_uniform(val, lower_bound, upper_bound):
  ...
  if lower_bound <= val <= upper_bound:
    pdf = 1.0 / (upper_bound - lower_bound)
  else:
    pdf = 0.0
  # AKHIRI KODE DI SINI
...
def breed_bird_prediction(X, features, params_dict, probs_dict):
    ...
    posterior_breed_0 = probability_given_class(
        X, features, 0, params_dict
    ) * probs_dict[0]

    posterior_breed_1 = probability_given_class(
        X, features, 1, params_dict
    ) * probs_dict[1]

    posterior_breed_2 = probability_given_class(
        X, features, 2, params_dict
    ) * probs_dict[2]
