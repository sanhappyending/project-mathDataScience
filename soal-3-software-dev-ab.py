...
Original file is located at
    https://colab.research.google.com/drive/1C0IL1tsG2fy9XSIDYKm6GivcHzPPOqcD
...
def estimate_sample_size_proportions(p1, p2, alpha=0.05, beta=0.20, two_sided=True):
    ...
    n = (
        (
            z_alpha * np.sqrt((1 + k) * p_bar * q_bar)
            + z_beta * np.sqrt((k * p1 * q1) + (p2 * q2))
        ) ** 2
    ) / (k * (delta ** 2))
...
# MULAI KODE DI SINI
# Hitung total sample yang diperlukan (dua kelompok) lalu hitung jumlah hari yang dibutuhkan
total_required = 2 * sample_size_needed
n_days = math.ceil(total_required / active_users_per_day)
# AKHIRI KODE DI SINI
...
def generate_df_ab_test(n_days):
    ...
    varian_dict = {
        "user_id": user_ids[n_origin:n_origin + n_varian],
